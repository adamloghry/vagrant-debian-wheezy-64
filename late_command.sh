...
# install salt - key is originally from http://debian.saltstack.com/debian-salt-team-joehealy.gpg.key but
# we do not want to pull data like this over unencrypted http subject to man in the middle attacks
echo "deb http://debian.saltstack.com/debian wheezy-saltstack main" >>  /etc/apt/sources.list.d/wheezy-saltstack.list
apt-key add - <<SALT_TEAM_KEY
...
SALT_TEAM_KEY
apt-get update
apt-get -y install salt-minion salt-doc python-augeas
